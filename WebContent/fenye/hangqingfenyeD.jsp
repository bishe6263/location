...
<div class="blank"></div>
<div class="pagesbox">
    <form name="selectPageForm" action="getquobycondition.spring?id=${id}&pp=${pinpai}&lx=${lx}&scgy=${scgy}&year=${year}" method="get">
        <div class="pager_record">总共<font class="f1">${total}</font>个记录</div>
        <div class="pagebar">
              <a href="<c:url value="getquobycondition.spring?id=${id}&pp=${pinpai}&lx=${lx}&scgy=${scgy}&year=${year}&page=${page-1>1?page-1:1}"/>">< 上一页</a>
                 <%--中间页--%>
              ...
              <!--   遍历       -->
                 <c:forEach begin="${beginpage}" end="${endpage}" varStatus="loop">
                  <c:set var="active" value="${loop.index==page?'page_now':''}"/>
                    <a class="${active}" href="<c:url value="getquobycondition.spring?id=${id}&pp=${pinpai}&lx=${lx}&scgy=${scgy}&year=${year}&page=${loop.index}"/>">${loop.index}</a>
                </c:forEach>
            
            <%--中间页结束,这是下一页--%>
            <a href="<c:url value="getquobycondition.spring?id=${id}&pp=${pinpai}&lx=${lx}&scgy=${scgy}&year=${year}&page=${page+1<totalPages?page+1:totalPages}"/>">下一页></a>
